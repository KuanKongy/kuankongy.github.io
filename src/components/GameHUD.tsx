...
function NextPiecePreview({ piece }: { piece: keyof typeof TETROMINOES | null }) {
  if (!piece) return <div className="h-12 w-12 rounded bg-white/5" />;
...
function Heart({ filled }: { filled: boolean }) {
  return (
    <svg
      ...
      viewBox="0 0 24 24"
      aria-hidden
      className={filled ? "text-tetra-z" : "text-white/20"}
      fill="currentColor"
    >
...
export default function GameHUD() {
  ...
  return (
    <div
      className={`dark pointer-events-none fixed inset-x-0 top-0 z-20 flex items-start justify-between px-4 pt-20 transition-opacity duration-500 md:px-8 ${
        visible ? "opacity-100" : "opacity-0"
      }`}
      aria-hidden={!visible}
    >
      <div className="frosted px-5 py-3">
        <div className="font-arcade text-[10px] tracking-widest text-tetra-i">
          SCORE
        </div>
        <div className="font-arcade text-3xl text-white tabular-nums">
          {String(score).padStart(5, "0")}
        </div>
        <div className="mt-1 flex items-center gap-3 text-xs text-white/70">
          <span>
            <span className="text-white/40">Height</span>{" "}
            <span className="font-arcade text-[11px] text-tetra-s">
              {height.toFixed(1)}u
            </span>
          </span>
          <span>
      ...
      <div className="flex flex-col items-end gap-3">
        {mode === "SURVIVAL" ? (
          <div className="frosted flex items-center gap-2 px-4 py-2">
            <span className="mr-1 font-arcade text-[10px] text-tetra-z">LIVES</span>
          ...
          <div className="frosted flex items-center gap-2 px-4 py-2">
            <span className="font-arcade text-[10px] text-tetra-i">MODE</span>
            <span className="font-arcade text-xs text-tetra-i">ENDLESS · ∞</span>
          </div>
        )}
        <div className="frosted flex items-center gap-3 px-4 py-3">
          <span className="font-arcade text-[10px] text-tetra-t">NEXT</span>
